...
//uploads post to the post collection
  uploadPost(File image, String location, String description) async {
    ...
    ref.set({
      ...
      "location": location ?? "Social Learning",
...
//add the likes to the notfication collection
  ...
  Future<void> deletePost(String postId) async {
    try {
      ...
      QuerySnapshot commentSnapshot = await commentRef.doc(postId).collection("comments").get();
      for (QueryDocumentSnapshot commentDoc in commentSnapshot.docs) {
        await commentDoc.reference.delete();
      }

      // Delete the comment reference document
      await commentRef.doc(postId).delete();

      // Delete all notifications related to this post
      QuerySnapshot notificationSnapshot = await notificationRef.get();
      for (QueryDocumentSnapshot notificationDoc in notificationSnapshot.docs) {
        QuerySnapshot postNotifications = await notificationDoc.reference
            .collection('notifications')
            .where('postId', isEqualTo: postId)
            .get();
        for (QueryDocumentSnapshot postNotification in postNotifications.docs) {
          await postNotification.reference.delete();
        }
      }
